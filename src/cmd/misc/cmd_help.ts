...
cmd_help.callback = (cli, msg, args) => {
  ...
  if (args.length == 0) {
    ...
    switch (args[0].toLowerCase()) {
      case "channel":
        txt += "Comando _*&channel*_: ```"
        txt += `Por defecto, cuando este bot es agregado al servidor, éste podrá escuchar los comandos en todos los servidores a los cuales tiene acceso. `
